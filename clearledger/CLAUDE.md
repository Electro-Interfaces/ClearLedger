...
6. **Специализированные селекторы вместо общей модалки.** Клик по чипу фильтра
   открывает только свои функции и применяет их сразу: период → `PeriodControl`
   (пресеты + календарь диапазоном), сеть → `WorkspaceScopePopover.tsx`
   (регионы · станции/точки · панель «Выбрано»). Общий фильтр — по кнопке «Фильтры».
