...
async def main():
    ...
    async with async_session_factory() as s:
        ...
        for e in data['02-entries']:
            ...
            # Месяц закрыт, если проведено ЗАКРЫТИЕ СЧЕТОВ РЕЗУЛЬТАТА — проводки на 90.09
            # или 99. Одной «Регламентной операции» мало: начисление ЕНС и амортизация
            # тоже регламентные, и по ним июль объявлялся закрытым при непроведённом
            # закрытии месяца — а витрины рисовали по нему рентабельность 99,7 %.
            closing_acc = ('90.09', '99')
            if e.get('ВидДокумента') == 'Регламентная операция' and any(
                    str(e.get(side) or '').startswith(acc)
                    for side in ('СчетДт', 'СчетКт') for acc in closing_acc):
                # Момент закрытия — время последней такой операции месяца: пустой
                # `closed_at` у закрытого периода означал бы, что мы знаем статус, но
                # не знаем, когда он наступил.
