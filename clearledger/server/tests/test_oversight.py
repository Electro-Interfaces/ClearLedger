...
async def test_лента_компании_одна_для_всех(auth_client, db: AsyncSession):
    """Работу компании видят одинаково: и начальник, и рядовой.

    Раньше лента сужалась охватом надзора, и начальник отдела видел МЕНЬШЕ
    рядового — свою ветку вместо компании, — а рядовому разрез не открывался
    вовсе. Отбор по людям делается явным фильтром, а не молча по должности
    (решение МАГа 13.09.2026).
    """
    from app.models import Task, TaskType
    ...
    # Личная запись соседа в ленту компании не входит ни у кого.
    db.add(Task(company_id=cid, type_id=тип.id,
                title=f"[{метка}] личное б", status="open",
                stage_code="new", visibility="personal",
                author_id=люди["б"].id, assignee_id=люди["б"].id))
    await db.commit()
    ...
    оба = {f"[{метка}] работа а", f"[{метка}] работа б"}
    for кто, подпись in ((люди["а"], "рядовой"), (люди["средний"], "начальник отдела"),
                         (люди["крупный"], "начальник управления")):
        r = await лента(кто)
        assert r.status_code == 200, f"{подпись}: {r.text}"
        названия = {w["title"] for w in r.json()["work"]}
        assert оба <= названия, f"{подпись} не видит всю работу компании"
        assert f"[{метка}] личное б" not in названия, (
            f"{подпись} видит чужую личную запись"
        )
