...
(недостачи/излишки) из ТЧ Товары (только строки Количество<>КоличествоУчет).
Имена номенклатуры — из кеша CbNomenclature. Read-only к 1С.
...
async def main() -> None:
    ...
    async with OneCComClient(conn) as client:
        ...
        # строки-отклонения (факт ≠ учёт) — одним запросом по всей сети
        dev = await client.query_tabular(
            DOC, "Товары",
            select=["Ссылка", "Ссылка.Склад", "Номенклатура", "Количество", "КоличествоУчет", "Сумма", "СуммаУчет"],
            where="Т.Количество <> Т.КоличествоУчет", top=100000,
        )
    print(f"получено: инвентаризаций {len(hdr)}, строк-отклонений {len(dev)}, складов {len(wh)}")
    ...
    async with async_session_factory() as db:
        ...
        # агрегаты отклонений по документу (только склады магазина)
        ...
        for r in dev:
            ...
            d_qty = fact - uch
            d_amt = _num(r.get("Сумма")) - _num(r.get("СуммаУчет"))
            a = agg[ref]
            a["dev_pos"] += 1
            if d_qty < 0:
            ...
            if len(a["lines"]) < 500:
                a["lines"].append({
                    ...
                    "fact": round(fact, 3), "uchet": round(uch, 3), "dev": round(d_qty, 3),
                    "amount_dev": round(d_amt, 2),
                })
        ...
        for h in hdr:
            ...
            db.add(CbInventoryDoc(
                ...
                lines=(sorted(a["lines"], key=lambda x: x["amount_dev"]) or None),
            ))
            n += 1
        await db.commit()
    ...
    print(f"CbInventoryDoc: записано {n} документов (208), с отклонениями {with_dev}")
