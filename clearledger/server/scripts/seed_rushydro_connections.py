...
SOURCES = [
    ...
    ("stations_excel", "Справочник станций ЭЗС",
     "Справочник объектов ЭЗС (xlsx, лист «Станции»): паспорт станции (серийный, адрес, "
     "координаты, коннекторы, мощность, OCPP, бренд, владелец, HubEx). Нормализация в объекты.",
     "connected", {"sheet": "Станции"}),
]
...
CHANNELS = [
    ...
    ("stations", "Справочник станций ЭЗС",
     "Загрузка справочника (xlsx) → L1 RAW → нормализация паспорта → объекты (L2, "
     "type=ev_charging, типизированные колонки + канон-регион).",
     "active", "stations_excel", "stations", "Паспорт станций"),
]
