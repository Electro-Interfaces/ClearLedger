...
CHANNEL_TEMPLATES: list[ChannelTemplateDecl] = [
    ...
    ChannelTemplateDecl(
        ...
        description=(
            "Ручные таблицы (xlsx) реестров по ЭЗС — файлы грузятся по очереди, формат "
            "определяется автоматически: «Сводная» (договоры э/э и аренды + помесячные "
            "объёмы входящей э/э), «ЭЗС_Договоры_Аренда» (актуальная постоянная часть "
            "аренды, ИНН и контакты контрагентов), «Тарифы Электроэнергия_Входящие» "
            "(входящие тарифы помесячно). Загрузка файла → L1 RAW → нормализация "
            "(сопряжение со станциями по № БУ/зав. номеру/координатам) → L2 (контрагенты/"
            "договоры/платёжная дисциплина/объёмы и тарифы э/э) → витрины «Энергозакупка» "
            "и «Аренда»."
        ),
        icon="Table",
        direction="energy",
        status="available",
        streams=[
            # Один канал — три файла-слота (config.uploadFiles, формат автодетектом):
            # якорная «Сводная» прописывает станциям buNumber/zoi1, остальные
            # резолвятся по ним каскадом.
            StreamDecl("manual_table", "contracts_payments", "anchor", "Сводная: договоры + объёмы э/э"),
            StreamDecl("manual_table", "rent_contracts", "reference", "Договоры аренды (актуальные)"),
            StreamDecl("manual_table", "energy_tariffs", "reference", "Тарифы э/э входящие"),
        ],
        stages=_STAGES_INGEST,
