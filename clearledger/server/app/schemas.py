...
class CompanyBrief(BaseModel):
    ...
    # Смотрит ли человек за чужой работой в этом пространстве: администратор —
    # за всей, руководитель подразделения — за своей веткой, рядовой — ни за
    # кем. От этого зависит, показывать ли раздел «Компания»: окно, через
    # которое ничего нельзя сделать, рядовому только мешает.
    oversees: bool = False
...
class MeResponse(BaseModel):
    ...
    # Когда человека можно трогать: пояс IANA и рабочее окно. Регламентная
    # сводка приходит в начале дня и в середине, в тишину не прорывается.
...
class CounterpartyDocBrief(BaseModel):
    """Последний документ контрагента (для карточки в «Контрагентах»).

    `id` нужен просмотрщику: по номеру с датой документ не найти — пара не
    уникальна (два разных №1-2212 от 22.12.2023 у разных контрагентов).
    """
    id: str | None = None
    docType: str
    label: str | None = None
