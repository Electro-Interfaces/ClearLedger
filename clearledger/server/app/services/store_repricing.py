...
Что здесь принципиально другое — право. Станция меняет только то, что ей
разрешено (price_owner = station). Центр, наоборот, распоряжается ЦЕНТРАЛЬНЫМИ
позициями, а станционные не трогает: иначе решение АЗС о местной цене молча
затиралось бы сетевым правилом, и никто бы не понял, откуда взялась цена на
полке.
...
ЧТО_ДЕЛАТЬ = {
    ЦЕНТР: "право на цену отдано станции — она меняет её у себя, центр видит результат",
...
async def preview(db: AsyncSession, cid, правило: dict, отбор: dict,
    ...
    for (sid, uuid_), c in цены.items():
        ...
        if (c["price_owner"] or "master") == "station":
            новая, отказ = старая, ЦЕНТР
        elif kvi and not трогать_kvi:
