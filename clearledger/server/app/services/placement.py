...
об одном предмете: в какой моей кучке он лежит, когда я им занимаюсь и важен ли
...
async def put(db: AsyncSession, company_id: uuid.UUID, user_id: uuid.UUID,
    ...
    if list_id is not None:
        # Кучка эксклюзивна: назначение новой вытесняет прежнюю, а не добавляет
        # вторую. Пустой UUID сюда не приходит — «убрать из кучки» это `clear`
