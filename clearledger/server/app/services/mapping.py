...
_REGION_CANON = {
    ...
    # Субъекты, встретившиеся в выгрузке рынка 13.09.2026. Без них строка
    # возвращалась как есть, и «Волгоградская обл» жила отдельно от
    # «Волгоградской области», а «Моск. обл.» — отдельно от «Московской области»:
    # один субъект дробился на несколько территорий, и доля считалась по куску.
    "волгоградская": "Волгоградская область", "вологодская": "Вологодская область",
    "архангельская": "Архангельская область", "мурманская": "Мурманская область",
    "новгородская": "Новгородская область", "псковская": "Псковская область",
    "калининградская": "Калининградская область", "курская": "Курская область",
    "белгородская": "Белгородская область", "тамбовская": "Тамбовская область",
    "орловская": "Орловская область", "ивановская": "Ивановская область",
    "костромская": "Костромская область", "кировская": "Кировская область",
    "ульяновская": "Ульяновская область", "астраханская": "Астраханская область",
    "томская": "Томская область", "крым": "Республика Крым",
    "севастополь": "Севастополь", "калмыкия": "Республика Калмыкия",
    "дагестан": "Республика Дагестан", "ингушетия": "Республика Ингушетия",
    "чеченская": "Чеченская Республика", "чечня": "Чеченская Республика",
    ...
    # Сокращения, которыми пишут два столичных субъекта.
    "моск": "Московская область", "мо": "Московская область",
    "подмосковье": "Московская область", "спб": "Санкт-Петербург",
    "санкт": "Санкт-Петербург", "петербург": "Санкт-Петербург",
...
def canon_region(raw) -> str | None:
    ...
    # Точка в сокращении («Моск. обл.») делала первое слово «моск.» и срывала
    # опознание: субъект возвращался в том виде, в каком пришёл.
...
_CITY_PREFIX = ("городской округ ", "гор. округ ", "го ", "г.о. ",
                "г.", "г ", "город ", "с.", "с ", "село ", "п.", "п ", "пос.", "пос ",
...
_BRAND_CANON = {
    ...
    # Те, кого в словаре не было: их написания разошлись в реестре и фильтр
    # показывал одного вендора двумя строками — «Терра-ток» (5 станций) и
    # «Терра-Ток» (3). Выбрав одну, человек терял часть сети (Чурилов, 12.09.2026).
    "терраток": "Терра-ток", "terratok": "Терра-ток",
    "schneider": "Schneider", "шнайдер": "Schneider",
    "wallbox": "Wallbox", "воллбокс": "Wallbox", "валлбокс": "Wallbox",
    "zarevo": "ZAREVO", "зарево": "ZAREVO",
