...
GATES: dict[str, list[dict[str, Any]]] = {
    "lead": [
        {"key": "address", "label": "Адрес и место установки", "field": "address_any"},
        {"key": "source", "label": "Кто предоставил площадку", "manual": True},
    ],
    "screening": [
        {"key": "coords", "label": "Координаты на карте", "field": "lat"},
        {"key": "place_kind", "label": "Тип места (город / трасса)", "field": "place_kind"},
        {"key": "demand", "label": "Оценка спроса и конкурентов рядом", "manual": True},
    ],
    "negotiation": [
        {"key": "owner", "label": "Собственник установлен", "field": "owner"},
        {"key": "contact", "label": "Контакт представителя получен", "manual": True},
        {"key": "consent", "label": "Принципиальное согласие", "manual": True},
        {"key": "rate", "label": "Ориентир по ставке аренды", "field": "rent_rate"},
    ],
    "dd": [
        {"key": "power", "label": "Свободная мощность, кВт", "field": "free_power_num"},
        {"key": "tp_cost", "label": "Стоимость техприсоединения", "field": "tp_cost"},
        {"key": "tp_term", "label": "Срок мероприятий, мес.", "field": "tp_term_months"},
        {"key": "control", "label": "Форма контроля участка", "field": "control_form"},
        {"key": "legal", "label": "Право проверено (ЕГРН, ВРИ, обременения)", "manual": True},
    ],
    "decision": [
        {"key": "capex", "label": "Затраты на подключение посчитаны", "field": "connection_cost"},
        {"key": "plan", "label": "План ЭЗС и мощности", "field": "planned_power_kwt"},
        {"key": "verdict", "label": "Инвестиционное решение принято", "manual": True},
    ],
    "contracting": [
        {"key": "contract", "label": "Договор подписан", "field": "contract_start"},
        {"key": "term", "label": "Срок договора зафиксирован", "field": "contract_end"},
    ],
    "construction": [
        {"key": "contractor", "label": "Подрядчик определён", "field": "contractor"},
        {"key": "tp_done", "label": "Техприсоединение исполнено", "manual": True},
        {"key": "smr", "label": "СМР завершены", "manual": True},
    ],
    "live": [
        {"key": "location", "label": "Объект заведён в реестре сети", "field": "location_id"},
    ],
}
...
def gate_state(site: EzsSite, stage: str | None = None) -> dict[str, Any]:
    """Чек-лист текущей стадии: что закрыто, что нет."""
...
async def create_site(db: AsyncSession, company_id, payload: dict[str, Any],
    ...
    site = EzsSite(company_id=company_id, stage=payload.get("stage") or "lead",
                   stage_since=date.today().isoformat(),
...
async def set_stage(db: AsyncSession, site: EzsSite, stage: str, *, reason: str | None,
                    user: User | None) -> dict[str, Any]:
    """Перевод стадии вручную. Незакрытый гейт не блокирует, но фиксируется."""
    if stage == site.stage:
        return {"moved": False, "gate": gate_state(site)}
    prev = site.stage
    gate = gate_state(site, prev)
    ...
    return {"moved": True, "missing": missing, "gate": gate_state(site)}
...
async def set_gate_item(db: AsyncSession, site: EzsSite, key: str, done: bool,
    ...
    return {"ok": True, "gate": gate_state(site)}
...
def site_out_full(site: EzsSite, owner_name: str | None = None) -> dict[str, Any]:
    """Карточка + поля ведения (Волна 2)."""
    out = _site_out(site)
    out.update({
