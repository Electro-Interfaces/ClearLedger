...
# Код валюты источника. Расшифровано по часовым поясам точек полной выгрузки
# (12 250 строк, 13.09.2026): 1 — рубли (6 139 точек), 3 — Грузия (10, Тбилиси),
# 6 — Молдова (112, Кишинёв и Бухарест). Коды 4 и 5 встречаются по одному разу и не
# расшифрованы — такая цена в сравнение не идёт, и это правильнее догадки.
#
# Рубль — единственная валюта, сравнимая с нашей ценой: смешение превратит медиану
# рынка в выдумку.
CURRENCY_BY_ID = {"1": "RUB", "3": "GEL", "6": "MDL"}
...
def in_russia(row: dict) -> bool:
    ...
    Поле `country` для этого НЕ годится, хотя и заполнено: в полной выгрузке оно
    равно «Россия» у всех 12 250 точек, включая Киев, Хельсинки и Токио. Это
    параметр обхода, а не факт о точке (проверено 13.09.2026). Единственный
    надёжный признак — часовой пояс: он отсекает 3 145 зарубежных точек, из них
    1 608 киевских и 773 финских.
    """
...
def parse_tariffs(raw: str | None) -> list[dict[str, Any]]:
    ...
    for chunk in (raw or "").split("|"):
        ...
        if price is None:
            continue
        out.append({
            # Тип берётся строго до первого двоеточия: иначе «08» из окна «@08:00»
            # станет отдельным тарифом. Кроме киловатт-часа источник знает парковку,
            # бронь, поминутную оплату, разовый платёж и бесплатность — полная
            # стоимость зарядки к цене киловатт-часа не сводится, и эти типы
            # сохраняются как есть, а не отбрасываются.
            "unit": {"per_kwt": "kwh", "per_kwh": "kwh", "per_minute": "minute",
                     "per_session": "session"}.get(unit.strip(), unit.strip()[:32]),
            "price": price,
...
def site_class(icon_type: str | None, operator: str | None) -> str:
    """Класс точки: сеть, независимая публичная или домашняя розетка.

    Три класса, а не два (полная выгрузка 13.09.2026). Кроме сетей и домашних
    розеток в реестре есть 2 013 российских точек с публичным видом, но БЕЗ
    оператора: их кто-то поставил и открыл для всех, а сети за ними нет.

    Записать их в сети нельзя — у них нет оператора, и доля рынка считалась бы по
    несуществующей компании. Записать в домашние розетки тоже нельзя — они реально
    обслуживают проезжающих и занимают место на территории. Поэтому отдельный класс:
    в предложении территории они есть, в сравнении сетей — нет.
    """
    ...
    if operator:
        return "network"
    # Публичный вид без оператора — независимая точка.
    return "independent" if known else "unknown"
