...
def _streak(shifts: list[dict]) -> dict:
    """Серия чистых дней подряд — тот самый критерий перехода.

    Считается от свежего дня назад, а не по всей истории: сверка обязана
    отвечать «сколько дней подряд сходится СЕЙЧАС», иначе одно апрельское
    расхождение закрывает критерий навсегда, и он перестаёт быть измеримым.

    Единица счёта — ДЕНЬ, а не смена. День чистый, когда чиста каждая его
    смена: на 208 их бывает две, и засчитывать день по одной сошедшейся значит
    закрывать глаза на вторую.

    Что не рвёт серию, потому что не говорит о качестве агента:
      · нет пакета ЦБ — его выгружают руками, молчание ничего не значит;
      · «нет данных» — ЦБ прислал пакет без документа продаж, сравнивать нечего.
    Что рвёт: расхождение и отсутствие пакета агента (смена была, а он её не
    собрал — это пропуск).
    """
...
def analyze_stock(doc: dict) -> dict:
    ...
    for r in cash_rows:
        bc = str(r.get("ШтрихКод") or "")
        qty = float(r.get("Остаток") or 0)
        cash_by_barcode[bc] += qty
        # «Товар есть, продать нельзя»: в кассе ноль, в учёте — остаток.
        if qty <= 0 and bc and book_by_barcode.get(bc, 0) > 0:
            cannot_sell.append({"ns_code": r.get("КодНС"), "barcode": bc,
                                "name": r.get("Наименование"),
                                "book_qty": round(book_by_barcode[bc], 3)})
    ...
    missing_in_cash = []
    for bc, qty in book_by_barcode.items():
        if qty > 0 and bc and bc not in cash_by_barcode:
            missing_in_cash.append({"barcode": bc, "book_qty": round(qty, 3)})
