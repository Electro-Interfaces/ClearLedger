    ...
    rows = (await db.execute(text("""
        WITH src AS (
            ...
            SELECT CASE WHEN p.source LIKE 'Edge%' OR p.source LIKE 'TradeLedger%'
                   ...
                   -- выглядел трёхкратно полнее, чем есть.
                   --
                   -- Ключ документа: у продаж — смена (один сводный документ на
                   -- смену, опознаётся внутренним номером кассы), у остальных —
                   -- собственный номер. На ИсточникUUID полагаться нельзя: при
