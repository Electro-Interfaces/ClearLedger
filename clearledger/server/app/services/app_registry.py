...
_PULSE_MODULES: list[tuple[str, str]] = [
    ...
    ("business.myline", "Бизнес · Моя линия"),
    ("business.shift", "Бизнес · Смена"),
    ("business.objects", "Бизнес · Где болит"),
    ("business.support", "Бизнес · Поддержка"),
    ("business.summary", "Бизнес · Коротко"),
    # Бухгалтерское пространство: те же права, другой предмет разрезов.
    ("business.money", "Бизнес · Деньги"),
    ("business.accounting", "Бизнес · Учёт"),
    ("business.taxes", "Бизнес · Налоги"),
    ("business.requests", "Бизнес · Чего ждём"),
    ("business.deadlines", "Бизнес · Сроки"),
    ("business.tasks", "Бизнес · Задачи"),
...
# Разделы «Сайта». Данные держит САЙТ — он публичен и должен работать, даже когда
# пространство лежит. Здесь их читают (кто написал, кого завели, кому открывали стенд)
# и правят витрину: её тексты и цены перестали быть кодом и стали данными сайта.
_SITE_MODULES: list[tuple[str, str]] = [
    ("leads", "Заявки"),
    ("requests", "Обращения"),
    ("cabinets", "Кабинеты"),
    ("demos", "Демо-доступы"),
...
_APPS: list[dict[str, Any]] = [
    ...
    # Рабочее место развития сети, а не заготовка под рассылки: рынок вокруг нас,
    # территории, цена и направления роста (13.09.2026).
    {"code": "marketing", "name": "Маркетинг", "icon": "megaphone", "sort": 12,
     "desc": "Рынок вокруг сети: карта и компании, территории, цена, куда расти",
     "modules": []},
    ...
    # кто считает выручку. Мост на действующий хаб ElsyPlus Link (03.09.2026): парк ГИГ
    # ведётся там же — сеть gig-net, узлы АКАЗС и 208, heartbeat идёт на публичный адрес.
    # Прежде продукт стоял «в подключении» с заставкой: движок был, плитки не было.
    {"code": "netlink", "name": "Сеть передачи данных", "icon": "network", "sort": 22,
     "base_url": "https://link.dataworker.ru",
    ...
    # «Дело» — документооборот пространства. Документ здесь самостоятельный объект
    # с регистрационным номером и реестром, а не файл при задаче: из вложения
    # поручения не построить журнал входящих за квартал. Поручение по документу
    # ставится «Задачами» — второй движок работы не заводим.
    {"code": "docs", "name": "Трек", "icon": "folder-open", "sort": 32,
     "desc": "Документооборот и работа компании: реестры входящих и исходящих, "
             "приказы, согласование, поручения и регламент",
     "modules": []},
    ...
    {"code": "econ", "name": "Экономика", "icon": "trending-up", "sort": 22,
     ...
     "modules": []},
    # «Сайт» — рабочее место того, кто ведёт публичную витрину компании (elsyplus.ru):
    # обращения из кабинета, кто заведён и кому открыт демо-стенд, что показано клиенту.
    # Сайт живёт своим сервером и своей базой и остаётся ХОЗЯИНОМ этих данных — продукт
    # читает их коннектором, а внутрь пространства течёт только обращение, становясь
    # заявкой Поддержки. Второго кабинета мы не заводим: у сайта уже есть вход по коду,
    # уровни доступа и журнал стендов, и дублировать их в Ядре — заводить вторую правду.
    {"code": "site", "name": "Сайт", "icon": "globe", "sort": 33,
     "desc": "Публичная витрина и кабинет клиента: обращения, кабинеты, демо-доступы",
     "modules": _SITE_MODULES},
    {"code": "elsy", "name": "Элси+", "icon": "handshake", "sort": 34,
     "desc": "Работа с нами, документы, продукты и демонстрации", "modules": []},
    # «Периметр» — всё, чем компания владеет и что должна помимо баланса. Первые
    # два слоя (забалансовые счета и невидимое в балансе) считает «Бухгалтерия», и
    # там же они остаются: это учётный вопрос. Здесь они собраны вместе с третьим
    # слоем — устными договорённостями, обещаниями и решениями, которых в учёте нет
    # вовсе. Смысл продукта в том, чтобы официальное и неофициальное лежали рядом,
    # но не смешивались: у каждой записи третьего слоя видно, чем она подтверждена.
    {"code": "perimeter", "name": "Периметр", "icon": "shield", "sort": 23,
     "desc": "Обязательства и имущество вне баланса: забалансовые счета, "
             "невидимое в балансе и договорённости без документа",
     "modules": []},
    ...
    {"code": "data", "name": "Данные", "icon": "database", "sort": 60,
     ...
     "modules": []},
    # «Аудитор» — сквозной агент пространства: его зовут из любого экрана панелью
    # справа и кнопкой в шапке, а плитка ведёт на собственную страницу (каталог
    # навыков и разговор во всю ширину). Продукт в реестре нужен затем, чтобы право
    # на агента выдавалось поимённо, как на любое рабочее место: данные он видит
    # ровно те же, что спросивший, но спрашивать дают не каждому.
    # Требует сервиса `auditor` в стеке (ecosystem-deploy/services/auditor).
    {"code": "auditor", "name": "Аудитор", "icon": "bot", "sort": 28,
     "desc": "Спросить про данные пространства словами: проверки учёта, закрытие "
             "периода, долги и находки по текущему экрану",
     "modules": []},
...
_CARVED_BY_PROFILE: dict[str, set[str]] = {
    ...
    # «Трек» — документооборот, работает во всех профилях.
    "energy": {"projects", "ops", "sales", "corp", "shop", "marketing", "finance", "data", "connect",
               "netlink", "accounting", "diag", "docs"},
    ...
    # «Трек» подключён рознице 16.08.2026: обкатка на пилоте сети и в офисе прошла
    # (визы, номенклатура дел, обмен с СЭД, ознакомление), держать рознице отдельный
    # порядок работы с бумагами незачем.
    # «Сеть передачи данных» подключена рознице 03.09.2026 по решению МАГа: движок за
    # ней встал — ElsyPlus Link ведёт 12 узлов сети, дашборд и удалённый доступ живые.
    "fuel": {"sales", "shop", "ops", "finance", "data", "connect", "docs", "netlink"},
    ...
    # «Аудитор» включён офису первым: у аудиторской практики вопрос «что не так в
    # этих данных» — основная работа, а не украшение. Сетевым профилям он подключается
    # отдельной записью, когда под них наберутся свои навыки.
    # «Сайт» — только у офисного профиля: публичная витрина есть у компании, которая
    # продаёт свои продукты, а не у сети ЭЗС или АЗС, работающей на своего заказчика.
    "office": {"books", "revenue", "econ", "perimeter", "data", "connect", "auditor", "docs",
               "site"},
    # Производственное пространство (ElsyPlus): компания, которая ДЕЛАЕТ продукты, а не
    # ведёт объекты и не считает чужую бухгалтерию. Работа идёт вокруг своей команды —
    # «Трек» (документы и поручения), «Сайт» (публичная витрина) и «Сеть передачи
    # данных» (свой Link). Бухгалтерии здесь нет намеренно (решение МАГа 07.09.2026):
    # ни «Учёта», ни «Бухгалтерии», ни «Реализации» — пустые витрины про чужую работу.
    # «Поддержка» и «Аудитор» подключаются записью, как и в остальных профилях.
    "works": {"docs", "site", "netlink"},
}
...
# «Задачи» здесь больше нет: поручения переехали в «Трек» (решение МАГа
# 16.08.2026). Продукт остаётся в каталоге — его ручки обслуживают поручения
# внутри «Трека», — но отдельной плиткой в лаунчере не показывается: два места
# для «моей работы» человек читает как два разных списка дел.
_ALWAYS_ON = {"admin", "chat", "conf", "info", "pulse"}
...
_BY_PROFILE: dict[tuple[str, str], tuple[str, str]] = {
    ...
    # это и есть отраслевой разрез, рядом с ним «Торговый центр» (сопутка) читается сразу.
    ("sales", "fuel"): ("Топливо",
                        "Сеть АЗС: обзор и карта, аналитика, тарифы, корпоратив и частные "
                        "лица, товародвижение"),
    # ⚠ В центре продукт зовётся «Торговый центр», а не «Магазин» (словарь цепочки,
    # решение МАГа 02.09.2026): «Магазин» — это рабочее место НА СТАНЦИИ, в офисе
    # магазина нет. Одно слово в двух смыслах путало сильнее, чем два разных.
    ("shop", "fuel"): ("Торговый центр",
