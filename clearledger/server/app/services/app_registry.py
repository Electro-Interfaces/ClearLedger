...
# Пункты «Пульса» как модули приложения. Нужны не для «покупки» частей продукта,
# а для отбора: директор решает, ЧТО из картины видит куратор — руководитель
# верхнего уровня, которому не нужен весь разбор. Ключи попадают и в конструктор
# роли («Управление» → роли), и в фильтр самого «Пульса».
_PULSE_MODULES: list[tuple[str, str]] = [
    ("today", "Экран дня"),
    ("accepted", "Принятое сегодня"),
    ("sources", "Источники данных"),
    ("targets", "Пороги эскалаций"),
    ("business.sales", "Бизнес · Продажи"),
    ("business.projects", "Бизнес · Проекты"),
    ("business.ops", "Бизнес · Эксплуатация"),
    ("business.contacts", "Бизнес · Обращения"),
    ("business.objects", "Бизнес · Где болит"),
    ("business.support", "Бизнес · Поддержка"),
    ("business.summary", "Бизнес · Коротко"),
    ("team.people", "Команда · Люди"),
    ("team.departments", "Команда · Подразделения"),
    ("team.access", "Команда · Доступ"),
    ("team.comms", "Команда · Переписка"),
    ("week.totals", "Неделя · Итоги"),
    ("week.moves", "Неделя · Движения"),
]
...
_APPS: list[dict[str, Any]] = [
    ...
    # «Пульс» — рабочее место руководителя (ecosystem-deploy/docs/PULSE.md): экран дня
    # с эскалациями-сводками и KPI по направлениям, действия с телефона. Круг узкий —
    # роли выдаются поимённо; плитка стоит сразу после «Управления».
    {"code": "pulse", "name": "Пульс", "icon": "activity", "sort": 8,
     "desc": "Рабочее место руководителя: эскалации, KPI дня, команда, неделя",
     "modules": _PULSE_MODULES},
    ...
    # ── Рабочие места компании без объектов (профиль `office`) ──────────────────
    # Первоисток: у такой компании главный источник данных — её БУХГАЛТЕРИЯ. Закрытый
    # период неизменяем и служит эталоном, «Продажи» и «Услуги» своих чисел не заводят
    # — читают тот же регистр под своим углом (см. routers/books_router.py).
    # Коды свои, а не `accounting`/`sales`: те заняты продуктами сетевых профилей, и
    # одно имя на два разных рабочих места развело бы маршруты и права.
    {"code": "books", "name": "Бухгалтерия", "icon": "book-open", "sort": 20,
     "desc": "Эталон учёта: обороты, проводки, документы, закрытые периоды",
     "modules": []},
    # «Реализация» — одно рабочее место на оба разреза выручки (решение МАГа
    # 13.08.2026). Раньше это были два приложения, «Продажи» и «Услуги», с одним и
    # тем же экраном: человек ходил между плитками, чтобы задать один вопрос — что
    # компания продала за период. Товары и услуги стали РАЗДЕЛАМИ продукта, как
    # «Сеть» и «Коммерция» у «Топлива» (`config/spaceProducts.ts`).
    # «Экономика» — левая половина отчёта, которой у пространства не было: куда
    # компания тратит и сколько зарабатывает. Строится на тех же проводках, что и
    # «Бухгалтерия», но отвечает не «что в регистре», а «сколько осталось»: ОФР,
    # структура затрат, налоги и нагрузка.
    {"code": "econ", "name": "Экономика", "icon": "trending-up", "sort": 22,
     "desc": "Результат компании: отчёт о финансовых результатах, расходы, "
             "рентабельность, налоги и нагрузка",
     "modules": []},
    {"code": "revenue", "name": "Реализация", "icon": "bar-chart-3", "sort": 21,
     "desc": "Что продали и оказали: динамика, покупатели и заказчики, "
             "номенклатура, документы реализации",
     "modules": []},
    ...
    {"code": "connect", "name": "Подключения", "icon": "cable", "sort": 59,
     ...
     "modules": [
         ...
         # Имя было «Источники и коннекторы», а в меню — «Коннекторы»: в матрице
         # «Роли и доступ» строка называлась иначе, чем пункт, которым её выдают.
         ("connectors", "Коннекторы"),
         # Реквизиты доступа к внешним системам. Раздела не было ни в реестре, ни в
         # карте прав — экран открывался прямой ссылкой любому участнику компании,
         # включая внешнего подрядчика, и показывал адреса и логины систем.
         ("sources", "Источники"),
    ...
    # «Задачи» — работа компании: поручения, согласования, регламентные работы. Заявки
    # сюда не переезжают: заявка описывает поломку у заявителя и живёт в контуре
    # Поддержки со своим SLA и внешними сторонами (решение МАГа 06.08.2026,
    # ecosystem-deploy/docs/TASKS.md). Код продукта остаётся `plan` — он смысловой,
    # за ним уже менялся движок.
    {"code": "plan", "name": "Задачи", "icon": "list-checks", "sort": 40,
     "desc": "Работа компании: поручения, согласования, маршруты и сроки", "modules": []},
...
_CARVED_BY_PROFILE: dict[str, set[str]] = {
    ...
    # Офис — компания без объектов, кроме собственного (аудит, консалтинг, услуги).
    # Разрез ПУСТОЙ, и это не заглушка: продукты энергетики и розницы построены вокруг
    # ЖИЗНИ ОБЪЕКТА (строим → эксплуатируем → продаём), а объекта здесь нет. Работа идёт
    # вокруг КЛИЕНТА, и её ведут рабочие места Ядра (Задачи, Пульс, Чаты — `_ALWAYS_ON`)
    # плюс Поддержка, которая включается явной записью, как и у остальных профилей.
    #
    # Сама запись в карте гасит плитку «Учёт»: его разделы — смены, ТТН, магазин,
    # общепит, обмен с 1С — показали бы офисной компании пустые витрины про то, чего у
    # неё нет. «Финансы» ей тоже не включаются: продукт про топливо и розницу.
    #
    # Свои рабочие места идут от бухгалтерии-эталона: «Бухгалтерия» (регистр целиком)
    # и «Реализация» (те же данные под углом продаж — товары и услуги двумя разделами
    # одного продукта). «Данные» — та же служебная кухня, что у сетевых профилей: карта
    # нормализованной базы и коннекторы, только домен в карте свой
    # («Бухгалтерия-эталон», см. services/space_data_model.py). «Подключения» — трубы,
    # по которым данные приходят: у компании без объектов ВЕСЬ учёт приезжает извне
    # (выгрузка 1С, живая база, ЭДО, банк, почта), поэтому вопрос «откуда что берётся»
    # здесь острее, чем у сетевых профилей, где половину данных даёт своё оборудование.
    "office": {"books", "revenue", "econ", "data", "connect"},
}
...
_ALWAYS_ON = {"admin", "chat", "plan", "conf", "info", "pulse"}
...
def carved_products(profile_id: str | None) -> set[str]:
    """Продукты разреза для профиля (пусто = своих продуктов у профиля нет)."""
    return _CARVED_BY_PROFILE.get(profile_id or "", set())


def is_carved(profile_id: str | None) -> bool:
    """Профиль описан картой разреза, то есть «Учёт» плиткой не показывается?

    Не то же, что непустой `carved_products`: у офисной компании (без объектов) своих
    продуктов в SPA нет вовсе, но и единый «Учёт» ей не нужен — его разделы про сеть,
    смены и товар. Раньше признаком служила непустота набора, и такой профиль молча
    получал полный «Учёт».
    """
    return (profile_id or "") in _CARVED_BY_PROFILE
...
def _default_app_on(code: str, profile_id: str | None = None) -> bool:
    ...
    if code in _ALWAYS_ON:
        return True
    if code == "ledger":
        return not is_carved(profile_id)
    # Профиль без разреза сюда тоже попадает: набор пуст, и продукты разреза не
    # показываются никому — у него они живут разделами внутри «Учёта».
    return code in carved_products(profile_id)
