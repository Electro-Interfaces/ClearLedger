...
_ADMIN_MODULES: list[tuple[str, str]] = [
    ...
    ("apps", "Приложения и модули"), ("objects", "Объекты"),
...
# Продукты пространства, заведённые «на вырост»: место в реестре и на столе есть, своих
# экранов пока нет — за маршрутом заставка. Разрезом Учёта они не являются (движки свои:
# Link — связь объектов, 1С — бухгалтерия), но включаются тому же профилю.
_SETUP_PRODUCTS = {"netlink", "accounting", "diag"}
...
def _default_app_on(code: str, profile_id: str | None = None) -> bool:
    ...
    if code in _CARVED_PRODUCTS or code in _SETUP_PRODUCTS:
        return carved
    if code == "ledger":
        return not carved
    return False
