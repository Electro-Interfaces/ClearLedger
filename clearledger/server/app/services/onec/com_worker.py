...
# Зона данных ЭЛСИ.АЗК — Europe/Moscow (UTC+3, без переходов с 2014).
# ⚠ pywin32 отдаёт даты 1С как pywintypes.datetime с ФИКТИВНОЙ меткой UTC
# ("GMT Standard Time"), хотя стенные часы в них — МЕСТНЫЕ московские. Из-за
# этого .isoformat() давал «07:24:02+00:00» для события, случившегося в 04:24:02
# UTC → мгновение уезжало на 3 часа, а у смен с ночным стартом (00:05 МСК =
# 21:05 UTC пред. дня) расходилась и ДАТА смены с топливным контуром (тот пишет
# истинный UTC). Поэтому переклеиваем метку на +03:00, не трогая стенные часы.
...
def op_fetch_cb_shifts(period_from: str, period_to: str,
    ...
    q.Текст = (
        ...
        f"И (Т.Склад.Код = &Код ИЛИ Т.Подразделение.Код = &Код) "
