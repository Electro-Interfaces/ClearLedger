...
# ВидДоговора: COM отдаёт СИНОНИМ («С поставщиком»), OData — имя значения
# («СПоставщиком»). Нормализуем к стабильному коду (имени значения перечисления),
# чтобы kind не зависел от транспорта. См. SCHEMA_REFS_GIG §2 (ВидыДоговоровКонтрагентов).
_CONTRACT_KIND_BY_SYNONYM = {
    "с покупателем": "СПокупателем",
    "с поставщиком": "СПоставщиком",
    "с комитентом (принципалом) на продажу": "СКомитентом",
    "с комиссионером (агентом) на продажу": "СКомиссионером",
    "с комитентом (принципалом) на закупку": "СКомитентомНаЗакупку",
    "с комиссионером (агентом) на закупку": "СКомиссионеромНаЗакупку",
    "с факторинговой компанией": "СФакторинговойКомпанией",
    "с транспортной компанией (доставка и оплата)": "СТранспортнойКомпанией",
    "заём полученный": "ЗаемПолученный",
    "заем полученный": "ЗаемПолученный",
    "прочее": "Прочее",
}


def _norm_contract_kind(raw: Any) -> str | None:
    """Синоним/имя ВидаДоговора → стабильный код. Неизвестное — как есть."""
    if not raw:
        return None
    s = str(raw).strip()
