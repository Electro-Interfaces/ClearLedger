...
SYSTEM_ROLES_FUEL: list[dict] = [
    {"name": "Полный доступ", "modules": None},
    # Коммерция сети плюс договоры и аренда: и то и другое ведёт один человек. Договор
    # приходит и уходит бумагой, поэтому «Трек» — часть того же рабочего места.
    {"name": "Финансист", "modules": ["sales", "ops", "docs"]},
    {"name": "Бухгалтер", "modules": ["finance", "docs"]},
    {"name": "Оператор данных", "modules": ["data"]},
...
# Ключа `plan` в ролях больше нет: «Задачи» сняты с лаунчера 16.08.2026, работа
# переехала в «Трек» (`docs`), и роль с мёртвым ключом открывала бы пустоту.
SYSTEM_ROLES_OFFICE: list[dict] = [
    {"name": "Полный доступ", "modules": None},
    # Руководитель практики: картина по людям и работам плюс канал с клиентами.
    {"name": "Руководитель", "modules": ["pulse", "support", "chat", "books", "revenue", "docs"]},
    ...
    {"name": "Специалист", "modules": ["chat", "conf", "docs"]},
