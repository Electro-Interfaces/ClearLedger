...
# половину забывают. Поэтому здесь ровно то, где забывают.
#
...
#
# **Наборы разные по профилю пространства.** Сеть ЭЗС не пишет «Задача
# разработки», а разработка не согласовывает земельный участок; общий список на
# всех означал бы, что каждое пространство половину заготовок листает мимо.
# Профиль тот же, что решает состав приложений (`_CARVED_BY_PROFILE`): пилот
# РусГидро — `energy`, наше пространство — `office`.

# Общее для любого пространства: обращение приходит всюду, где есть «Поддержка».
STARTER_TEMPLATES_COMMON: list[dict] = [
    {
        ...
        "description": "Пришло из «Поддержки». Сначала отвечаем человеку, "
                       "потом решаем, что это по существу.",
        "checklist": [
            "Воспроизвести у себя",
            "Решить, что это: вопрос, настройка или дефект",
            "Ответить заявителю тем же днём",
            "Если дефект — завести отдельную работу и связать с обращением",
        ],
    },
]

# Сеть ЭЗС: работа идёт по объекту — от участка до ввода и обслуживания.
# Формулировки взяты из их же процессов («Ветка проекта ЭЗС», «Работы на ЭЗС»)
# и воронки проекта (оформление земли → реализация → пусконаладка → эксплуатация).
STARTER_TEMPLATES_ENERGY: list[dict] = [
    {
        "name": "Согласование земельного участка",
        "title": "Земля: ",
        "type_code": "approval",
        "due_days": 30,
        "description": "Право на участок под ЭЗС: аренда, сервитут или "
                       "разрешение на размещение. Держит весь проект.",
        "checklist": [
            "Определить форму оформления: аренда, сервитут, разрешение",
            "Собрать документы на участок и проверить обременения",
            "Согласовать с собственником и получить его решение",
            "Подписать договор или получить разрешение",
            "Приложить документ к карточке проекта — по нему считается гейт",
        ],
    },
    {
        ...
        "type_code": "approval",
        "due_days": 60,
        "description": "От заявки в сетевую до акта: мощность, ТУ, договор, "
                       "выполнение. Сроки внешние, поэтому фиксируем каждый шаг.",
        "checklist": [
            "Подать заявку в сетевую организацию",
            "Получить технические условия и проверить мощность под проект",
            "Заключить договор технологического присоединения",
            "Выполнить мероприятия со своей стороны",
            "Получить акт об осуществлении ТП",
            "Внести номер и дату акта в карточку проекта",
        ],
    },
    {
        "name": "Ввод ЭЗС в эксплуатацию",
        "title": "Ввод: ",
        "type_code": "approval",
        "due_days": 14,
        "description": "Пусконаладка закончена — станция принимается в сеть. "
                       "Дата ввода это основание перевода капвложений 08 → 01.",
        "checklist": [
            "Проверить, что обязательные пункты проекта закрыты",
            "Провести пусконаладку и зафиксировать её результат",
            "Подписать акт ввода в эксплуатацию",
            "Убедиться, что станция отвечает в сети и видна в мониторинге",
            "Зафиксировать дату ввода — по ней считается учёт",
        ],
    },
    {
        "name": "Работы на площадке",
        "title": "Работы: ",
        "type_code": "errand",
        "priority": "high",
        "due_days": 3,
        "description": "Выезд на станцию: ремонт, замена оборудования, "
                       "обслуживание. Закрывается фактом, а не обещанием.",
        "checklist": [
            "Согласовать доступ на площадку и время работ",
            "Проверить наличие запчастей и инструмента до выезда",
            "Выполнить работы и зафиксировать, что именно сделано",
            "Проверить станцию под нагрузкой после работ",
            "Приложить фотографии и закрывающий документ",
        ],
    },
]

# Разработка: то, где забывают версию, признак готовности и живую проверку.
STARTER_TEMPLATES_OFFICE: list[dict] = [
    {
        "name": "Разбор ошибки",
        "title": "Ошибка: ",
        "type_code": "incident",
...
_TEMPLATES_BY_PROFILE: dict[str, list[dict]] = {
    "energy": STARTER_TEMPLATES_ENERGY,
    "office": STARTER_TEMPLATES_OFFICE,
}


def starter_templates(profile_id: str | None) -> list[dict]:
    """Что сеять этому пространству: общее плюс профильное.

    Неизвестный профиль получает только общее — это честнее, чем отдать ему
    чужой набор: заготовка, которой не пользуются, засоряет меню «Создать»
    ровно так же, как её отсутствие оставляет его пустым.
    """
    return STARTER_TEMPLATES_COMMON + _TEMPLATES_BY_PROFILE.get(profile_id or "", [])
...
async def list_tasks(
    ...
    if scope == "open":
        ...
        # или закрыть. Разрез надзорный: разбирает тот, кто отвечает за людей,
        # а рядовому здесь нечего решать — он и взять-то может только своё.
        from app.services import oversight
...
async def create_starter_templates(
    ...
    # Набор берётся по профилю пространства: сеть ЭЗС не пишет «Задачу
    # разработки», а разработка не согласовывает земельный участок.
    company = await db.get(Company, cid)
    specs = starter_templates(company.profile_id if company else None)

    needed = {spec["type_code"] for spec in specs}
