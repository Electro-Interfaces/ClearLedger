...
_QUERY_TOKEN = re.compile(r'#(\S+)|(\w+)\s*:\s*("[^"]*"|\S+)', re.UNICODE)
...
async def _parse_query(db: AsyncSession, cid: uuid.UUID, user: User,
    ...
    for m in _QUERY_TOKEN.finditer(text):
        ...
        if flag:
            key = _QUERY_FLAGS.get(flag.lower())
            (parsed.update({key[0]: key[1]}) if key else unknown.append(f"#{flag}"))
            continue

        name = _QUERY_FIELDS.get((field or "").lower())
        ...
        if name == "project":
            ...
            code = next((st.get("code") for ty in types for st in _route_of(ty)
            ...
            elif (due := _cmd_due(value)) is not None:
                parsed["due_to"] = due
            else:
