...
class SetupField:
    ...
    secret: bool = False
    # Группа поля для структурирования формы в UI (карточки-секции).
    # None → поле в общей секции «Параметры».
    group: str | None = None
...
class SourceAdapter(ABC):
    ...
    # Версия адаптера/интеграции (семвер-строка). Растёт при изменении контракта
    # выгрузки/полей. UI показывает рядом со статусом — чтобы было понятно,
    # какая версия логики обработки привязана к источнику.
    version: str = "1.0"

    # Подробная инструкция подключения («что запросить у провайдера, как получить
    # данные»). Показывается ТОЛЬКО в панели настройки источника, НЕ в тайле
    # каталога — чтобы каталог оставался компактным. description = одна строка.
    setup_guide: str = ""
