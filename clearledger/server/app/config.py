...
class Settings(BaseSettings):
    ...
    # Сервер записи конференций кладёт готовый mp4 в пространство этим ключом.
    # Он общий на все пространства и подтверждает только «файл прислал наш jibri»:
    # кому именно принадлежит комната, решает её имя, которое считается на
    # приватном ключе пространства и не угадывается.
    conf_ingest_token: str = ""
    # Закрыть конференцию для всех можно только со стороны XMPP-сервера: выгнать
    # участников не может даже организатор. Рядом с ним стоит приёмник, который
    # уничтожает комнату по нашей просьбе; пусто — «Завершить» закроет запись в
    # журнале, а комната останется жить до последнего участника.
    conf_closer_url: str = ""
