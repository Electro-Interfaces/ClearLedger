...
class UserCompany(Base):
    ...
    # Кем человек работает В ПРИЛОЖЕНИИ: {"support": "developer"}.
    #
    # Роль пространства отвечает на вопрос «что ему можно здесь» и знает два
    # значения — админ и остальные. У службы поддержки этого мало: оператор,
    # менеджер, разработчик и инженер делают разную работу и видят разные
    # рабочие места, а в пространстве все они одинаковые `user`. Раньше роль в
    # приложении можно было поправить только в его собственной базе — и она
    # молча возвращалась назад при следующей синхронизации состава.
    #
    # Пусто — роль считается по карте приложения, как раньше.
