...
export const SALES_COMMERCE_MENU: CentralMenuItem[] = [
  ...
  // Отбракованное источником. Отдельным пунктом, а не видом внутри «Расхождений»:
  // там разбирают спор данных между собой, здесь — то, чему сам источник отказал
  // в доверии, и в витринах этих строк нет вовсе.
  { key: 'cs_rejected',  label: 'Отбракованные' },
]
...
export const REV_MONEY_MENU: CentralMenuItem[] = [
  ...
  // Банк идёт ПЕРЕД свёрнутыми витринами: поток и статьи — производные от операций,
  // а сами операции и остатки по счетам продукт не показывал вовсе. Бухгалтер
  // работает именно этим разрезом («что прошло по банку»), и сверять деньги
  // с выпиской банка больше нечем.
  { key: 'rev_bank',     label: 'Счета и остатки' },
  { key: 'rev_statement', label: 'Выписка' },
...
export const PER_PICTURE_MENU: CentralMenuItem[] = [
  ...
  // возможностей. Отдельного пункта «Сроки» больше нет — он показывал то же самое
  // (просроченные договорённости, пропущенные периоды, выдачи с прошедшим сроком),
  // и объяснить разницу между двумя экранами было нельзя.
  { key: 'pr_review', label: 'Что требует решения' },
]
...
export const PER_CASH_MENU: CentralMenuItem[] = [
  ...
  // Мост изменения долга, прогноз потребности и сверка кошелька: на эти вопросы
...
export const PER_PEOPLE_MENU: CentralMenuItem[] = [
  // Один список вместо двух: «Расчёты с людьми» и «Люди» считали по человеку одно и
  // то же — операции, выданное, остаток, очередь на оформление — и лежали в разных
  // разделах, так что нужный приходилось угадывать. Отсюда же открывается сверка.
