...
export const MODULE_COMPONENTS: ModuleComponent[] = [
  ...
  // ── СКВОЗНОЕ · ИТОГИ (видно только после проводок) ──────────────────────────
  {
    ...
    section: 'acc_results',
    label: 'Маржа',
    description: 'Две маржи рядом: управленческая по FIFO-партиям (наш расчёт, виден сразу) и бухгалтерская из проводок 90.01/90.02 (появляется после проведения в БП). Расхождение между ними — предмет разговора.',
    profiles: ['fuel'], status: 'active',
