...
export const MODE_LABELS: Record<CoreMode, string> = {
  ...
  rev_stock: 'Реализация · Закупки',
  rev_help: 'Реализация · Помощь',
...
const SUB_LABELS: Partial<Record<CoreMode, Record<string, string>>> = {
  ...
  store_receipt: Object.fromEntries(storeMenu('store_receipt').map((m) => [m.key, m.label])),
  ...
  rev_stock: Object.fromEntries(REV_STOCK_MENU.map((m) => [m.key, m.label])),
  rev_help: Object.fromEntries(REV_HELP_MENU.map((m) => [m.key, m.label])),
