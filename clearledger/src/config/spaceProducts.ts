...
export interface SpaceProduct {
  ...
  modes: CoreMode[]
  /** Раздел, с которого продукт открывается без `?mode=`. По умолчанию — первый из `modes`. */
  defaultMode?: CoreMode
  /**
...
export const SPACE_PRODUCTS: SpaceProduct[] = [
  ...
  },
  {
    ...
    // Открывается на «Сессиях» (первый пункт — «Разрезы»): продажи смотрят по сессиям,
    // а «Сеть» отвечает на вопрос о составе парка и нужна реже (решение МАГа 10.09.2026).
    defaultMode: 'sales_sessions',
  },
  {
  ...
  {
    // Маркетинг — рынок вокруг сети: что происходит вокруг и что нам с этим делать.
    // Четыре раздела (docs/MARKET-ROADMAP.md §8): рынок, территории, цена и позиция,
    // решения. Все четыре режима обязаны быть перечислены здесь: в оболочке продукта
    // рельса показывает ТОЛЬКО заявленные им режимы, и незаявленный раздел просто не
    // появляется — экраны при этом есть и работают, что и сбивает с толку.
    code: 'marketing', route: '/marketing', label: 'Маркетинг',
    modes: ['marketing', 'marketing_area', 'marketing_price', 'marketing_decision'],
    paths: [],
  },
  {
  ...
  },
  {
    ...
    // «Периметр» — обязательства и имущество компании помимо баланса, все три слоя
    // сразу. Первые два (забалансовые счета и невидимое в балансе) считает и
    // показывает «Бухгалтерия»; здесь те же витрины стоят рядом с третьим слоем —
...
const ROUTE_APPS: [string, string][] = [
  ...
  ['/tickets', 'support'], ['/tasks', 'plan'], ['/docs', 'docs'],
