...
// можно было закрепить и возвращаться к нему одним кликом. Разделы табуются как
// обычные экраны — человек закрепляет «Моя работа» и «Доску» рядом.
for (const [to, label] of [['/tasks', 'Задачи'], ['/tasks/company', 'Работа компании'],
  ['/tasks/overview', 'Обзор задач'], ['/tasks/setup', 'Настройка задач']] as const) {
