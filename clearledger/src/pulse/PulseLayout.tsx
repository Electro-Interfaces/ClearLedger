...
export const PULSE_VIEWS: Record<string, PulseView[]> = {
  ...
  '/pulse/business': [
    ...
    // Два пункта контакт-центра вблизи: «Обращения» меряют неделю компании, эти —
    // текущую минуту. «Моя линия» — единственный экран «Пульса» про самого
    // смотрящего: оператор отвечает за свои разговоры, а не за счёт всей смены.
    { key: 'myline', label: 'Моя линия', hint: 'что на мне сейчас и успеваю ли я по норме' },
    { key: 'shift', label: 'Смена', hint: 'кто на линии прямо сейчас и где затор' },
