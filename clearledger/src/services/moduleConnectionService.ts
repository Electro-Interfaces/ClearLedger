 ...
 *
 * ⚠ Это ВТОРОЙ уровень — под-модули внутри раздела (витрины, пункты меню, компоненты).
 * Видимость самих РАЗДЕЛОВ живёт на сервере: реестр Ядра (`hooks/useCompanyRegistry.ts`
 * → `companyModules` в CompanyContext). Новые «что подключено компании» заводить здесь
 * не нужно — им место в реестре, иначе настройка снова окажется в браузере одного человека.
 */
