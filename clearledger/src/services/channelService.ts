...
export async function runChannel(
  id: string,
  period?: { dateFrom?: string; dateTo?: string; stationCodes?: number[]; allPeriod?: boolean; mode?: 'append' | 'replace' | 'all' },
): Promise<any> {
  ...
  const body = period
    ? {
        ...
        // режим табличной загрузки: 'replace' переписать (сессии/станции) /
        // 'all' переобработать все файлы-слоты (реестры) / 'append' подгрузить
