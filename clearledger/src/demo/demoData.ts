...
export const DEMO_INFO_ARTICLES = [
  ...
  },
  {
    ...
    bodyMd: '# Сетевой инцидент в Треке\n\nОператор создаёт карточку **INC-2471**, связывает её с АЗС Лесная и назначает инженера. В дело помещаются акт первичной диагностики, уведомление провайдеру и распоряжение о временной работе через LTE.\n\nПоручения показывают сроки и ответственных, согласование фиксирует решение `lead.engineer`, а вся переписка содержит ссылку на один и тот же инцидент.',
    bindings: [{ appCode: 'docs', sectionKey: 'registry', weight: 110 }],
  },
  {
    ...
    bodyMd: '# Какие чаты использовать\n\n**Сеть АЗС** показывает общую оперативную картину, **Оперативные уведомления** — важные изменения режима, а группа **INC-2471 · АЗС Лесная** собирает диагностику и решения по конкретному инциденту.\n\nИтоговые сроки и документы фиксируются в Треке; в чате остаётся ссылка на карточку INC-2471.',
  ...
  },
  {
    ...
    bodyMd: '# Подключение бизнес-приложений\n\nПульс, Поддержка, Реализация, Корпоративный процессинг, Розничный магазин, Монитор, Экономика, Периметр, Бухгалтерия, Проекты, Эксплуатация, Сеть передачи данных, Диагностика, Интернет-магазин и Маркетинг подключаются компании отдельно.\n\nПосле подключения приложение использует уже настроенные роли, объекты, данные, чаты и документы пространства. В этом контуре карточки показывают каталог, а каждое прикладное приложение демонстрируется отдельным стендом.',
