...
import { locationToStationInfo } from './locationMapping';
import type { ServiceLocation } from '@/types/location';
import type { MSTOReconciliationParams, StationInfo } from '@/types/mstoReconciliation';
...
export function MSTOReconciliationParamsModal({
  ...
  const [selectedStationIds, setSelectedStationIds] = useState<number[]>([]); // номера станций STS
  ...
  // Станции с распознанным номером (StationInfo + Location для отображения)
  const availableStations = useMemo(
    () => locations
      .map(l => ({ location: l, info: locationToStationInfo(l) }))
      .filter((s): s is { location: ServiceLocation; info: StationInfo } => s.info != null),
    [locations]
  );

  // Обработчик переключения станции (по номеру STS)
  const toggleStation = (stsStationId: number) => {
    setSelectedStationIds(prev =>
      prev.includes(stsStationId)
        ? prev.filter(id => id !== stsStationId)
        : [...prev, stsStationId]
    );
  };
  ...
  const handleSubmit = () => {
    ...
    const stationsInfo = availableStations.map(s => s.info);
    ...
    const stationIdsToSend = allStations
      ...
      : selectedStationIds.filter(id => id > 0);
        ...
        <div className="relative px-4 sm:px-6 pt-5 sm:pt-6 pb-4 sm:pb-5 bg-gradient-to-br from-cyan-600/20 via-teal-600/10 to-background flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-0.5">MSTO ↔ Смена</p>
            </div>
          </div>
            ...
            {!allStations && (
              <div className="grid grid-cols-2 gap-2 max-h-[120px] overflow-y-auto p-3 rounded-xl bg-card/30 border border-border/50">
                {availableStations.length > 0 ? (
                  availableStations.map(({ location, info }) => {
                    const isSelected = selectedStationIds.includes(info.stsStationId);
                    return (
                      <div
                        key={location.id}
                        onClick={() => toggleStation(info.stsStationId)}
                      ...
                      >
                        ...
                        <span className={`text-sm truncate ${isSelected ? 'text-foreground' : 'text-foreground/80'}`}>
                          {location.name || `АЗС ${info.stsStationId}`}
                        </span>
                      </div>
                    );
                  })
