/**
 * Переключатель юрлица ВНУТРИ организации.
 *
 * В пространстве работают организации — их и выбирают в шапке (`CompanySelector`).
 * Здесь уровень ниже: у одной организации в базе 1С бывают ООО и ИП одного владельца,
 * и без этого выбора их НДС и прибыль складываются в одну цифру — тихо, без ошибки
 * на экране.
 *
 * 🔴 Показывается, ТОЛЬКО когда юрлиц действительно несколько.
 *
 * Раньше стоял всегда — из соображения «место не должно появляться и исчезать». На
 * практике вышло хуже: у организации с единственным юрлицом в шапке оказывались два
 * похожих переключателя с почти одинаковыми названиями («ПРОМИЗОЛ СПБ» и «ООО
 * ПРОМИЗОЛ СПБ»), и это читалось как ошибка системы. Выбор, которого нет, показывать
 * не надо: текущая организация видна в правом блоке шапки.
 */
...
export function OrganizationSelector() {
  ...
  const label = pinned ? pinned.name : active ? active.name : 'Все юрлица'

  // Выбирать не из чего — не занимаем место в шапке. Ни «Организация не заведена», ни
  // единственное юрлицо переключателем быть не должны: это не выбор, а подпись, и
  // подпись эта уже есть справа.
