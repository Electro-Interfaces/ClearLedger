...
import { Fuel, Settings, ArrowRight, ClipboardList, Truck, Zap, Activity } from 'lucide-react'
...
export function OnboardingScreen() {
  ...
  // Energy-профиль (ЭЗС): обезличенный TradeLedger без фуел/GIG/ТТН.
  const HeaderIcon = isEnergy ? Zap : Fuel
  const title = isEnergy ? 'TradeLedger' : 'GIG Fuel Ledger'
        ...
        <div className="relative bg-card border border-border rounded-2xl p-8 md:p-12 text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground leading-relaxed">
              {isEnergy ? (
                <>Управленческий учёт<br />по сети электрозарядных станций</>
              ) : (
            ...
            {isEnergy ? (
              <>
                <span className="flex items-center gap-1.5">
                  <Zap className="h-3.5 w-3.5" /> Сессии зарядки
                </span>
                <span className="flex items-center gap-1.5">
                  <Activity className="h-3.5 w-3.5" /> Телеметрия ЭЗС
                </span>
              </>
            ) : (
              <>
