    ...
    <div className="p-6 space-y-4 max-w-5xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
          Это очередь <b className="text-foreground">отклонённых заявок</b> на чужой код, а не
          список сломанных позиций. Действующих коллизий в справочнике нет и быть не может:
          активный штрихкод принадлежит ровно одной карточке, это держит уникальный индекс
          (на экране «Штрихкоды» тот же факт — «Дубли · 0»). Заявка появляется, когда станция
          привязывает код, уже занятый другой карточкой: код ей не достаётся, а карточка
          остаётся непробиваемой — и это ждёт решения. Автоматически их не разрешают, потому
          что оба исхода что-то ломают: передача снимает код у того, кто по нему сейчас
          торгует, отказ оставляет второй товар без кода.
        </p>
        {/* Разбор списка по существу: у большинства претензий имена совпадают —
            это не спор за код, а два дубля одного товара, и лечатся они
            слиянием, а не передачей. Пока цифра была одна на всех, человек
            видел сто одинаковых с виду карточек и не понимал, где настоящая
            работа. */}
        {list.length > 0 && (
          <p className="mt-2 text-xs text-muted-foreground">
            Из них <b className="text-foreground">{list.filter((c) => !c.same_item).length}</b>
            {' '}— спор за код разных товаров: решать передачей или отказом.
            {' '}<b className="text-foreground">{list.filter((c) => c.same_item).length}</b>
            {' '}— две карточки одного товара: передача кода их не разведёт, нужно слияние
            {' '}(«Товар и станции» → «Тёзки»).
            {' '}Претензии за удалёнными карточками сюда не попадают — решать там нечего.
            {' '}⚠ Признак «одного товара» — точное совпадение имени, поэтому один и тот же
            {' '}товар под разными написаниями («Антифриз Sibiria G11 green -40 5 кг.» и
            {' '}«Антифриз Sibiria G-11 зеленый 5 кг.») попадает в споры: настоящих споров
            {' '}меньше, чем показано.
          </p>
        )}
      </div>
