  ...
  return wrap(q, () => {
    ...
    return (
      <Shell title="Штрихкоды / EAN"
        sub={`${d.total} штрихкодов · ${Object.entries(d.by_type).map(([t, n]) => `${t}: ${n}`).join(' · ')}`}>
        <div className="mb-3">
        ...
        <Table
          ...
          rows={items.slice(0, 300).map((i) => [
            <span className="font-mono">{i.barcode}</span>, i.owner_name, i.type ?? '—', i.main ? 'да' : '',
          ])}
        />
        {items.length > 300 && (
  ...
  return wrap(q, () => {
    ...
    return (
      <Shell title="Рецептуры (ТТК)"
        sub={`${d.period.from} – ${d.period.to} · ${d.summary.count} блюд с рецептурой (разворот ТТК из продаж общепита)`}>
        {d.recipes.length === 0
          ? <div className="text-sm text-muted-foreground">Нет блюд с рецептурой за период (поставьте апрель).</div>
          : (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {d.recipes.map((r) => (
                <div key={r.name} className="rounded-lg border border-border/50 bg-card/40 p-3">
                  <div className="text-sm font-medium mb-1.5">{r.name} <span className="text-[10px] text-muted-foreground font-normal">· {r.ing_count} ингр.</span></div>
