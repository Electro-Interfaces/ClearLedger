...
export function WorkContextActions({ companyId, subjectRef, message, onClose }: {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !save.isPending) onClose() }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-md">
    <DialogHeader><DialogTitle>Приложить сообщение к проекту или объекту</DialogTitle><DialogDescription>Выберите предмет и что с ним сделать: добавить обсуждение, зафиксировать решение, положить файл в его документы. Сообщение останется основанием — в чате появится ссылка.</DialogDescription></DialogHeader>
    ...
    {context.data?.actions.length === 0 && <p className="text-sm text-muted-foreground">У этого предмета нет своих действий: приложить сообщение к нему можно только поручением или процессом — они в том же меню.</p>}
