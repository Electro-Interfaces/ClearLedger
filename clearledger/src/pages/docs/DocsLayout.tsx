...
export const DOCS_VIEWS: Record<string, DocsView[]> = {
  ...
  '/docs/work': [
    // «Сегодня» вне групп: это не разрез работы, а сводка дня, в которой есть
    // и взятое мной, и принесённое компанией. Поставить её в любую из групп
    // значило бы соврать в обе стороны.
    // Первые три — одно и то же: личная отметка на работе. «Сегодня» это
    // `taken_for`, «Важное» — `starred`, «Отложено» — `deferred_until`; ни одну
    // из них не видит никто, кроме хозяина, и ни одна ничего не меняет в самом
    // предмете. Заголовка у блока нет намеренно: это то, с чего начинают, а
    // всякое название здесь было бы жаргоном — «раскладка» им и оказалась.
    { key: 'today', label: 'Сегодня', hint: 'день целиком: что взял, что принесли, напоминания', badge: 'hot' },
    { key: 'starred', label: 'Важное', hint: 'помеченное лично: важность своя, приоритет предмета ставит постановщик', badge: 'starred' },
    { key: 'deferred', label: 'Отложено', hint: 'спрятанное у себя до даты — срок компании при этом не менялся', badge: 'deferred' },
    ...
    // Ждут от меня — то, чего я не могу отменить: очередь, виза, ознакомление.
    ...
    // Веду сам — моя работа. Подсказка «Поручений» обещала и порученное мной,
    // хотя экран показывает только то, что делаю я: порученное живёт в «Слежу».
    { key: 'errands', label: 'Поручения', hint: 'работа, которую выполняю я', group: 'Веду сам', badge: 'errands' },
    { key: 'mine', label: 'Мои документы', hint: 'где я автор или ответственный', group: 'Веду сам', badge: 'own' },

    // Слежу — чужая работа, за которой я смотрю и с которой спрошу.
    { key: 'assigned', label: 'Я поставил', hint: 'что поручил другим — с меня спросят результат', group: 'Слежу', badge: 'assigned' },
    { key: 'watching', label: 'Наблюдаю', hint: 'чужая работа, за которой слежу со стороны', group: 'Слежу', badge: 'watching' },
  ...
  // Три вопроса, а не девять строк подряд: что в работе, где она стоит, что
  // пришло извне или уже отработано.
  '/docs/company': [
    // «Вся работа» не отвечала на вопрос «вся — это какая»: рядом стоял реестр
    // поручений, и различить их можно было, только открыв оба.
    { key: 'work', label: 'Документы и поручения', hint: 'одной лентой, с общими колонками состояния', group: 'Что в работе' },
    { key: 'triage', label: 'Разбор', hint: 'работа без исполнителя: взять, поручить или закрыть', group: 'Что в работе', badge: 'triage' },
    { key: 'errands', label: 'Реестр поручений', hint: 'только поручения: отбор, поиск и выгрузка', group: 'Что в работе' },
    // Не «Документы на доске»: доска — это подача, а экран отвечает, где
    // застряла виза и кого ждут.
    { key: 'docs', label: 'Согласование', hint: 'где застряло согласование и кого ждут', group: 'Что в работе' },
    { key: 'work-board', label: 'Доска работы', hint: 'общие колонки: документы и поручения вместе', group: 'Где стоит' },
    { key: 'board', label: 'Доска поручений', hint: 'канбан по стадиям маршрута', group: 'Где стоит' },
    { key: 'plan', label: 'Планирование', hint: 'бэклог и спринт: что берём следующим', group: 'Где стоит' },
    { key: 'inbox', label: 'Приём из СЭД', hint: 'что головная компания положила нам в папку', group: 'Извне и потом' },
    { key: 'archive', label: 'Архив', hint: 'сроки хранения, запреты и акты уничтожения', group: 'Извне и потом' },
  ],
  '/docs/overview': [
