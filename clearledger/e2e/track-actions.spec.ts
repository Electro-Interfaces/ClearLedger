...
test('очередь: действия в строке меняют её', async ({ page }) => {
  ...
  if (await раскладка.count()) {
    ...
    // Продукт называет это «Не сегодня» — глаголом решения, а не именем
    // состояния: откладывают не предмет, а свой день.
    if (!/Не сегодня/.test(await page.locator('body').innerText())) {
      нашли('раскладка: в поповере нет «Не сегодня»')
    } else хорошо('«Не сегодня» доступно из поповера раскладки')
