...
| 2 | Счёт-фактура | primary/invoices | 80% |
| 3 | УПД | primary/invoices | 85% |
| 4 | Счёт на оплату | primary/invoices | 75% |
| 5 | Акт сверки | primary/acts | 85% |
| 6 | Акт приёма-передачи | primary/acts | 80% |
| 7 | Акт выполненных работ | primary/acts | 70% |
| 8 | Договор | primary/contracts | 80% |
| 9 | Паспорт качества | primary/quality | 85% |
| 10 | Изображение (fallback) | primary/ttn | 40% |
...
### Фаза 2 — Email + 1С парсеры

| Задача | Библиотека | Статус |
|--------|-----------|--------|
| .eml парсер (headers, body, вложения → связанные записи) | postal-mime (~15 КБ) | Заглушка |
| 1С XML парсер (CommerceML, EnterpriseData → маппинг типов) | fast-xml-parser (~35 КБ) | Заглушка |
| Дедупликация по Email Message-ID | — | Заглушка |
| Дедупликация по 1С GUID | — | Заглушка |

### Фаза 3 — Чаты + OCR + Excel

| Задача | Библиотека | Статус |
|--------|-----------|--------|
| OCR для изображений (русский язык) | tesseract.js (~160 КБ + 12 МБ lazy) | Заглушка |
| Excel/CSV/DBF парсер | xlsx / SheetJS (~280 КБ) | Заглушка |
...
- Email + 1С парсеры (Фаза 2)
- OCR + Excel парсинг (Фаза 3)
