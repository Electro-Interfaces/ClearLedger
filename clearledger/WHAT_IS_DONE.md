# ClearLedger — что заявлено и что реально в коде

> **23.05.2026 — disclaimer о расхождении с реальностью.**
>
> Этот документ описывает заявленный функционал по версиям v0.2–v0.6.2. **Ревизия 23.05 показала**, что часть заявок не соответствует коду:
>
> - ❌ **v0.6 «Живое подключение к 1С через OData + EnterpriseData XML»** — на бэке отсутствует. Заявленные `server/app/services/onec/odata_client.py`, `sync_service.py`, `file_exchange.py`, `scheduler.py` **не существуют**. Фронт `OneCConnectionForm.tsx` + `useOneCSync.ts` обращаются к несуществующим `/api/onec/*`.
> - ⚠️ **v0.6.2 Аудитор TSupport** — UI работает, но на демо-данных `DEMO_AUDIT_RESULT`. Бэк-сервиса аудита нет.
> - ⚠️ Alembic-миграции не настроены — таблицы создаются через `SQLAlchemy.create_all()` при старте.
> - ✅ Остальное (FastAPI auth, entries, audit-log, экспорт, intake, fuel models + STS-клиент) — соответствует коду.
>
> Идеология ClearLedger переосмыслена 23.05.2026 — см. `README.md` и план `C:\Users\magsp\.claude\plans\ethereal-tumbling-valley.md`. Этот документ оставлен как **архивная зарисовка фаз разработки v0.3–v0.6.2** для git-блейма и понимания истории. Не использовать как источник правды о текущем состоянии.

## Идея (актуальная редакция 23.05.2026)

ClearLedger — **корпоративная среда подготовки документов для менеджеров компании** до загрузки в бухгалтерию. Двусторонний симбиоз с БП через цикл «эталон закрытых периодов ↔ рабочая зона незакрытого». БП **тянет** готовое через своё расширение (`GIG_Ledger.cfe`), ClearLedger не пихает.

## Идея (старая редакция, оставлена для контекста)

ClearLedger — **буферная зона между реальным миром и 1С:Бухгалтерией**. Система приёма, верификации, анализа и подготовки документов для компаний с разными профилями деятельности. Три маркетинговых кейса ценности:

| Кейс | Суть | Статус |
|------|------|--------|
| **1. Приём и верификация** | Документы из любых каналов → сверка с 1С + внешние сервисы → подтверждено / ошибки | **v0.3 — реализуется** |
| **2. AI-анализ периода** | Непрерывная проверка по правилам клиента, сверки с контрагентами, нормализация | Следующий этап |
| **3. Моделирование закрытия** | Песочница для квартала, рекомендации по оптимизации учёта | Горизонт |

Текущая версия (v0.2) — клиентский прототип (localStorage + IndexedDB). Серверная архитектура в v0.3 (FastAPI + PostgreSQL).

> **Архитектурное решение (2026-02-26):** Docker-контейнер (FastAPI + PostgreSQL + nginx), три слоя данных: Layer 1 (RAW-файлы), Layer 1a (staging, скрыт от клиента), Layer 2 (public, нормализованные данные). AI-обработка на облачном сервере. Подробности → [LAYER2_ARCHITECTURE.md](../LAYER2_ARCHITECTURE.md)
