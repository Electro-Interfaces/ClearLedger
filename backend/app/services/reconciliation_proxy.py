"""
Прокси к внешним API TradeCorp и MSTO для модуля «Сверки».

Перенесён из TradeFrame (server/routes/tradecorp.js, server/routes/msto.js,
server/services/mstoProxyService.js). Смысл: секреты TradeCorp/MSTO остаются на
сервере (httpx), фронт ходит только в /api/tradecorp/* и /api/msto/*.

Токены кэшируются в памяти процесса (как в исходной Node-версии); TradeCorp ~55 мин,
MSTO ~23 ч. GET-ответы MSTO кэшируются с TTL по типу пути.
"""
...
def _tc_emitent() -> int:
    try:
        return int(settings.tradecorp_emitent_id)
...
async def _tc_refresh_token() -> None:
    ...
    resp = await _tc_client.post(
        ...
        json={
            ...
            "id": 0,
            "method": "login",
            "params": {"login": settings.tradecorp_login, "password": settings.tradecorp_password},
        },
    )
...
async def _tc_get_client() -> httpx.AsyncClient:
    global _tc_client
    if not (settings.tradecorp_api_url and settings.tradecorp_login and settings.tradecorp_password):
        raise RuntimeError("Missing required TradeCorp API environment variables")
    if _tc_client is None:
        _tc_client = httpx.AsyncClient(
            base_url=settings.tradecorp_api_url,
            timeout=60.0,
...
async def _msto_refresh_token() -> None:
    ...
    if not (settings.msto_username and settings.msto_password):
        raise RuntimeError("Missing required MSTO credentials")
    _msto_client.headers.pop("Authorization", None)
    _msto_token = None
    resp = await _msto_client.post("/session", json={"username": settings.msto_username, "password": settings.msto_password})
    resp.raise_for_status()
...
async def _msto_get_client() -> httpx.AsyncClient:
    ...
    if _msto_client is None:
        _msto_client = httpx.AsyncClient(
            base_url=settings.msto_api_url, timeout=90.0, headers={"Content-Type": "application/json"}
        )
...
def msto_token_info() -> dict[str, Any]:
    return {
        "tokenValid": bool(_msto_token),
        "tokenExpiry": datetime.utcfromtimestamp(_msto_token_expiry).isoformat() + "Z" if _msto_token_expiry else None,
    }
